...
public:	// メソッド
	...
	const COLORREF GetForeColor() const;
	...
	int GetNotifyMonth( const COleDateTime& cTimeNow) const;	// 通知日移動あるいは毎月による移動

	// 通知日
	int SetTimerDay( int nDay);
	int GetTimerDay( void) const;
	int GetNotifyDay( const COleDateTime& cTimeNow) const;		// 通知日移動による移動
	...
	BOOL SetAlphaBlend( BYTE byAlphaBlend);
...
protected:	// メソッド
	...
	int ExchangeNotifyDay( const COleDateTime& cTimeNow, const int nGetType) const;
...
protected:	// メンバー変数
	...
	CString*	m_pcStrContextID;	// 結果受信ID
	...
	union CREATEDATE
	{
		...
		struct
		{
			...
			UINT	m_btCreateYear:14;	// bit:000000000xxxxxxx-xxxxx00000000000
		};
