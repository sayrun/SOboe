...
CSOboeApp::CSOboeApp()
{
	...
	*/
	...
	for( nIndex = 0; nIndex < _NETEXLIB_MAX; nIndex++)
	{
...
BOOL CSOboeApp::LoadProfile( BOOL blAllLoad)
{
	...
	if( HFILE_ERROR != OpenFile( cStrProfile, &of, OF_READ | OF_EXIST))
	{
		...
		CString	cStrPath;
		CString	cStrMenu;
		for( nIndex = 0; nIndex < _CEL_MAX; nIndex++)
		{
		...
		GetPrivateProfileString( m_cStrUserName, cStrKey, "", szWork, 2048, cStrProfile);
		cStrMenu = szWork;
		for( nIndex = ( _CEL_MAX - cStrMenu.GetLength()); nIndex >= 0; nIndex--)
		{
			cStrMenu += '0';
		}
		...
		// ユーザー設定による定義色
		for( nIndex = 0; nIndex < ( _USERCOLOR_MAX - 2); nIndex++)
		{
