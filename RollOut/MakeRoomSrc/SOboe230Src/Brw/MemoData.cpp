...
BOOL CMemoData::GetTitle( CString& cStrTitle) const
{
	...
	if( m_cStrTitle.IsEmpty())
	{
		...
		if( 0 >= nIndex || _MAX_TITLE_LEN < nIndex)
		{
			nIndex = _MAX_TITLE_LEN;
		}
...
BOOL CMemoData::GetFont( LOGFONT* const pstLogFont) const
{
	if( NULL == m_pstLogFont)
	{
		...
		CBrwApp*	cBrwApp = ( CBrwApp*)AfxGetApp();
		if( cBrwApp)
		{
			cBrwApp->GetDefaultFont( pstLogFont);
		}
		return FALSE;
	}
...
BOOL CMemoData::GetForeColor( COLORREF& clrFore) const
{
	if( CLR_INVALID == m_clrForeColor)
	{
		...
		if( cBrwApp)
		{
			clrFore = cBrwApp->GetDefaultForeColor();
		}
		return FALSE;
	}
...
BOOL CMemoData::GetBackColor( COLORREF& clrBack) const
{
	if( CLR_INVALID == m_clrBackColor)
	{
		...
		if( cBrwApp)
		{
			clrBack = cBrwApp->GetDefaultBackColor();
		}
		return FALSE;
	}
...
// 通知日の内部変換
int CMemoData::ExchangeNotifyDay( const CTime& cTimeNow, const int nGetType) const
{
	CTime	cTimeCheck( cTimeNow.GetYear(), ( ( 0 != m_nMonth) ? m_nMonth : cTimeNow.GetMonth()), m_nDay, cTimeNow.GetHour(), cTimeNow.GetMinute(), cTimeNow.GetSecond());
	...
	case NOTIFYSHIFT_TOSAT:	// 土曜日へ移動
		if( 1 == nDayOfWeek)cTimeCheck -= CTimeSpan( 1, 0, 0, 0);
		break;
	case NOTIFYSHIFT_TOFRI:	// 金曜日へ移動
		if( 1 == nDayOfWeek)cTimeCheck -= CTimeSpan( 2, 0, 0, 0);
		if( 7 == nDayOfWeek)cTimeCheck -= CTimeSpan( 1, 0, 0, 0);
		break;
	case NOTIFYSHIFT_TOMON_FROMSATSUN: // 月曜日へ移動：土／日曜日
		if( 7 == nDayOfWeek)cTimeCheck += CTimeSpan( 2, 0, 0, 0);
...
CMemoData* CMemoData::CreateMemoFromBinData( BYTE* pabyData, int nSize)
{
	...
	{
		pcMemoData->m_cStrParameters = pabyData[ pstDDData->m_nszParam];
	}
	pcMemoData->m_blLink		= ( pcMemoData->m_cStrFile.IsEmpty() && pcMemoData->m_cStrParameters.IsEmpty()) ? FALSE : pstDDData->m_blLink;
	...
	{
		pcMemoData->SetDxlExtData( &pabyData[ pstDDData->m_nabyDxlExtData], pstDDData->m_unDxlExtDataSize);
	}
