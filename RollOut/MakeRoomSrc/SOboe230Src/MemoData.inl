...
inline BOOL CMemoData::IsTimerExec( void) const
{
	...
	if( IsLink())
	{
		return ( m_blTimerExec ? TRUE : FALSE);
	}
...
inline BOOL CMemoData::IsPassWordLock( void) const
{
	...
	return ( m_blPassWordLock ? TRUE : FALSE);
}
...
// 通知日移動あるいは毎月による移動
inline int CMemoData::GetNotifyMonth( const COleDateTime& cTimeNow) const
{
...
// 通知日移動による移動
inline int CMemoData::GetNotifyDay( const COleDateTime& cTimeNow) const
{
...
{
	return ( m_blAfterNotify ? TRUE : FALSE);
}
...
{
	return ( m_blSeeThrough ? TRUE : FALSE);
}
...
inline BOOL CMemoData::IsEqualContextID( LPCSTR lpcszID)
{
	if( NULL == m_pcStrContextID)return FALSE;
	return ( *m_pcStrContextID == lpcszID);
}

// 透過用のデータ
inline BYTE CMemoData::GetAlphaBlend( void) const
{
	return max( m_byAlphaBlend, 16);
}

inline BOOL CMemoData::SetAlphaBlend( BYTE byAlphaBlend)
{
	m_byAlphaBlend = max( byAlphaBlend, 16);
	return ( m_byAlphaBlend == byAlphaBlend);
}

// 直接参照ね。
inline const LOGFONT* CMemoData::GetLogFont() const
{
	return m_pstLogFont;
}

inline const COLORREF CMemoData::GetForeColor() const
{
	return m_clrForeColor;
}

inline const COLORREF CMemoData::GetBackColor() const
{
