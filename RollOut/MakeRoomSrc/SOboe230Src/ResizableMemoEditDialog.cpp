...
void CResizableMemoEditDialog::OnTemplet()
{
	...
	if( cTempletMemu.CreatePopupMenu())
	{
		...
		for( nIndex = 0; nIndex < EDITDLG_TOOLBAR; nIndex++)
		{
...
void CResizableMemoEditDialog::OnDxlsel() 
{
	...
	if( cDxlSelMemu.CreatePopupMenu())
	{
		...
		cDxlSelMemu.AppendMenu( ( 0 == m_cMemoData.GetDxlID()) ? MF_CHECKED | MF_BYCOMMAND | MF_STRING : MF_BYCOMMAND | MF_STRING, IDM_DXLITEMXX, "未使用");
		...
		for( int nIndex = 0; nIndex < nMaxEntry; nIndex++)
		{
			...
			if( pcDrawExLib)
			{
				if( pcDrawExLib->IsFunctionAssign())
				{
					...
					cDxlSelMemu.AppendMenu( ( pcDrawExLib->GetDxlID() == m_cMemoData.GetDxlID()) ? MF_CHECKED | MF_BYCOMMAND | MF_STRING : MF_BYCOMMAND | MF_STRING, IDM_DXLITEM00 + nIndex, cStrMenu);
				}
			}
		}
